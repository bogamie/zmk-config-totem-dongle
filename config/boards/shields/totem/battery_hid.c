 ...
 * battery levels to the host. Used by the zmk-totem-battery-tray daemon
 * on Linux to render battery percentages in the system tray.
 *
 * Lives on the dongle build only. Subscribes to ZMK's peripheral battery
 * events (which fire when CONFIG_ZMK_SPLIT_BLE_CENTRAL_BATTERY_LEVEL_FETCHING
 * is enabled) and pushes a 2-byte input report to a second USB HID
 * interface (HID_1) every time a value changes.
 *
...
LOG_MODULE_REGISTER(totem_battery_hid, CONFIG_ZMK_LOG_LEVEL);
...
static const uint8_t hid_report_desc[] = {
    ...
    0x09, 0x01,       /* Usage (0x01) — totem battery device            */
...
SYS_INIT(totem_battery_hid_init, APPLICATION, 91);
...
ZMK_LISTENER(totem_battery_hid_listener, battery_listener);
ZMK_SUBSCRIPTION(totem_battery_hid_listener, zmk_peripheral_battery_state_changed);
